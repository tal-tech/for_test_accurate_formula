...
TALError GetImageData(std::string &image_data, 
    ...
    if (!image_base64.empty()) {
        ...
        auto download_image = [&]()->bool {
            double img_size = 0.0;
            std::string err_msg;
            bool res = DownloadImage(image_url, image_data, img_size, err_msg, download_timeout_seconds);
            return res;
        };
        if(!CallWithRetry(retry_count, retry_sleep_millsecods, download_image)) {
...
TALError DecodeImage(cv::Mat &dest_img, const std::string &src_img) {
    ...
    if (!(16 < dest_img.rows && dest_img.rows < 4096) ||
        !(16 < dest_img.cols && dest_img.cols < 4096) ||
        (dest_img.rows > dest_img.cols && (dest_img.rows / dest_img.cols >= 15)) ||
        (dest_img.rows <= dest_img.cols && (dest_img.cols / dest_img.rows >= 15))) {
