...
struct CommonError {
    ...
    TALError E_UNKNOWN_REQ{service_code+1, "unknown input param error"};
    TALError E_REQ_BODY_JSON{service_code+2, "the request body is an illegal json"};
    TALError E_REQ_BODY_OBJECT{service_code+3, "the request body is not a json object"};
};
...
struct ImageCommonError : public CommonError {
    ...
    // 图像类通用错误码定义
    TALError E_IMAGE_URL_TYPE{service_code+101, "image_url type error"};
    TALError E_IMAGE_DOWNLOAD{service_code+102, "image download failed"};
    TALError E_IMAGE_BASE64_TYPE{service_code+103, "image_base64 type error"};
    TALError E_IMAGE_BASE64_DECODE{service_code+104, "image_base64 decode failed"};
    TALError E_REQ_IMAGE_NULL{service_code+105, "image_url and image_base64 cannot both be null"};
    TALError E_IMAGE_DECODE{service_code+106, "image decode failed"};
    TALError E_IMAGE_RESOLUTION{service_code+107, "unsupported image resolution"};
    TALError E_IMAGE_FORMAT{service_code+108, "unsupported image format"};
    TALError E_FACE_RECTS_NULL{service_code+109, "face_rectangles cannot be null"};
    TALError E_FACE_RECTS_TYPE{service_code+110, "face_rectangles type error"};
    TALError E_FACE_RECTS_ELEM_TYPE{service_code+111, "face_rectangles element type error"};
    TALError E_FACE_RECT_TYPE{service_code+112, "face rectangle type error"};
    TALError E_FACE_RECT_DATA_TYPE{service_code+113, "face rectangle data type error"};
    TALError E_FACE_RECT_DATA_INCOMPLETE{service_code+114, "face rectangle data is incomplete"};
    TALError E_FACE_RECT_NULL{service_code+115, "face rectangle cannnot be null"};
    TALError E_IMAGE_BASE64_NULL{service_code+116, "image_base64 cannot be null"};
};
