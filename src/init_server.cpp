...
void ConnectEureka() {
    ...
    std::string server_host{ConfParam::GetValue(CONF_HOST_NAME, "apollo-for-test-accurate-formula")};
    //GetHostName(server_host);
    TALInterface::SetAppVersion(server_host);
...
bool ReadConfigFile() {
    ...
    File file(FilePath(config_file), base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_READ);
...
void InitKafka() {
    ...
    std::string biz_type = ConfParam::GetValue(CONF_SERVICE_TYPE, "other");
...
void InitService() {
    ...
    if (g_current_env == CURRENT_ENV::LOCAL) {
        ReadConfigFile(); // 读取本地配置文件：只有LOCAL环境才会读取
    } else {
        ReadEnvConfig();
    }
...
void Listen(RequestEvents &events) {
    std::string app_name{ConfParam::GetValue(APOLLO_PAAS_EUREKA_APP_NAME, "UNDEFINED")};
    ...
    for (auto &event : events) {
        ...
        if (event.first.second == HTTP_METHOD::POST) {
            ...
            LOG(ERROR) << "unsupport http method: " << (int)event.first.second;
        }
    }
    ConnectEureka();
    int service_port = ConfParam::GetValue(APOLLO_LOCAL_SERVICE_PORT, 8889);
